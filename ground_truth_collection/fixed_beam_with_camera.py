"""Fixed-beam SNR collection with synchronized RealSense camera frames.

Same as `fixed_beam_snr_collection.py` (pins TX/RX to a single beam pair and
records SNR continuously) but also captures the RealSense colour stream so
each SNR row has a matching image timestamp. Used to build the YOLO training
dataset: paired (camera frame, BS-position SNR) examples for the offset MLP.

Run: `python3 fixed_beam_with_camera.py` (from this folder).
...
# Initialise the Intel RealSense colour stream at 640×480 @ 30 fps and
# return the started pipeline handle.
...
# RX thread: at a fixed beam pair, alternately samples IQ from the USRP and
# grabs a camera frame from `pipeline`, writing both to `experiment_dir`.
def rx_fixed_beam(usrp, rx_streamer, start_time, child_rx, child_tx, experiment_dir, SAMPLE_SIZE,pipeline):
    ...
    try:
        ...
        while True:
            ...
            # Save signal
            signal_filename = os.path.join(recv_signal_dir, f'tx{TX_BEAM}_rx{RX_BEAM}_iter{iter_num}.dat')
            recv_signal.astype(np.complex64).tofile(signal_filename)
            ...
            # Capture frame using RealSense
            frames = pipeline.wait_for_frames()
            color_frame = frames.get_color_frame()
            if color_frame:
                frame = np.asanyarray(color_frame.get_data())
                img_filename = os.path.join(image_dir, f'img_iter{iter_num:04d}.jpg')
                cv2.imwrite(img_filename, frame)
            else:
                img_filename = "N/A"
                print(f"[WARNING] No color frame received for iter {iter_num}")


            print(f"[{iter_num:03d}] SNR = {snr_db:.2f} dB | Noise = {noise_power_avg:.2f} dBm | ")
            iter_num += 1

    except KeyboardInterrupt:
        print("\nKeyboard interrupt received. Experiment stopped by user.")
        ...
        with open(timestamp_csv_filename, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Tx_Beam", "Rx_Beam", "Iteration", "SNR_dB", "Image_Filename"])
            writer.writerows(iteration_timestamps)
...
# Entry-point: initialise the RealSense camera + Sivers TX/RX, then kick off
# the RX/camera capture thread.
def run_fixed_beam_experiment(tx_streamer, rx_streamer, experiment_name, TX_BEAM, experiment_dir):
    ...
    rx_process = threading.Thread(
        target=rx_fixed_beam, args=(usrp, rx_streamer, start_time, child_rx, child_tx, experiment_dir, SAMPLE_SIZE,pipeline))
    rx_process.start()
...
if __name__ == "__main__":
    ...
    car_speed = input("Enter the car speed (e.g., 10mph): ").strip()
