"""Canonical full 64 × 64 TX × RX beam sweep on the rotor.

For every rotor angle in the sweep range, sweeps the full Sivers codebook
(64 TX × 64 RX beam pairs) and logs SNR per pair plus raw IQ buffers. This
is the **master ground-truth source** the VIBE pipeline compares against —
all per-runner orchestrators ultimately consume the CSVs derived from this
script's output (via `run_full_data_extraction.py`).

Two modes (selected interactively at startup):
- `Full Sweep` — 64 × 64 per rotor angle.
- `Single Sweep` — one chosen TX beam, 64 RX per rotor angle.

A camera-capture thread can be optionally enabled via the `VideoRecorder`
class to save a synchronized colour stream for YOLO dataset building.

Run: `python3 BeamSweeponRotor.py` (from this folder). Edit `BASE_DIR` in
the `__main__` block to point at your storage volume.
...
# video file in parallel with the radio sweep, used to build the YOLO dataset.
...
# RX thread: steps the rotor, then at each angle sweeps the full TX × RX
# codebook and logs per-pair SNR + raw IQ.
def rx_host(usrp, rx_streamer, start_time, child_rx, child_tx, sweep_mode, tx_beam, experiment_dir, sample_size):
    ...
    # Sweep from 0 to 180
    for target_angle in range(0,181, 1):   # forward rotation
        ...
        for tx_idx, tx_beam in enumerate(range(1, 64)):
            tic = time.time()
...
# `rx_host` (and optionally `VideoRecorder`), save metadata + heatmap on exit.
