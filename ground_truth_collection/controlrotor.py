"""Simple host-side rotor driver used by the ground-truth capture scripts.

Sends raw angle numbers (one per line, `<angle>\\n`) over the Arduino serial
link — older / simpler protocol than the `D:` / `C:` commands implemented by
`../combined_rotor_motion/combined_rotor_motion.ino`. Used here because the
step-and-pause sweeps don't need the continuous mode.
