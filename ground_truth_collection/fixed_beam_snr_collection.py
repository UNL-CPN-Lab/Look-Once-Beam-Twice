"""Fixed-beam SNR collection at a single stationary position (no rotor sweep).

Pins TX and RX to a fixed beam pair (`TX_BEAM` / `RX_BEAM`) and records SNR
over many iterations to characterize link stability and the noise floor.
Outputs per-iteration timestamps + SNR rows to CSV and the raw IQ buffer per
iteration to `recv_signal/`.

Run: `python3 fixed_beam_snr_collection.py` (from this folder).
...
# RX thread: streams IQ from the USRP at a fixed beam pair for many iterations
# and writes per-iteration SNR + timestamps + raw IQ to `experiment_dir`.
