"""Post-process raw rotor-sweep output into derived ground-truth CSVs.

Reads the per-rotor-angle folders written by `BeamSweeponRotor.py` (each
containing `snr_data.csv`) under `base_dir`, and writes the standard
ground-truth artifacts used by the online VIBE runners:

- `forward_all_snr_data.csv` — every (rotor angle, TX, RX) → SNR row
- `forward_max_snr_per_angle.csv` — max-SNR beam pair per rotor angle
- `tx<fixed>_max_snr.csv` — best-RX per angle for the most-frequent best-TX
- `snr_tx<fixed>_rx32_forward.csv` / `snr_tx32_rx32_forward.csv` — pinned-beam slices

Edit `base_dir` / `exp_dir` at the top before running.
...
tx_fixed_beam_index = tx_angle_index(tx_fixed)
print(f"[INFO] Most common Tx Beam Angle: {tx_fixed}")
...
for folder in sorted(os.listdir(base_dir)):
    ...
    if os.path.exists(snr_file_path):
        ...
        df = df[df["Tx Beam Index"] == tx_angle_index(tx_fixed)]
        if df.empty:
            continue
        max_row = df.loc[df["SNR (dB)"].idxmax()]
...
# Extract SNR-vs-rotor-angle for a single fixed (tx_index, rx_index) pair
# and write it to `output_filename` under both `base_dir` and `exp_dir`.
