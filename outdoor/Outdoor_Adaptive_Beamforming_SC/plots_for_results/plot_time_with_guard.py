...
# Thresholds and associated pastel colors
thresholds_info = {
...
# bf_plot_base = os.path.join(experiment_folder, "violin_beamforming_time_with_guards")
beams_plot_base = os.path.join(experiment_folder, "beams_checked_with_guards")
...
# )
# plt.title("Beamforming Time per Threshold with Guards", fontsize=16, fontweight="bold")
...
#     plt.savefig(f"{bf_plot_base}.{ext}", format=ext, dpi=600)
# plt.close()
...
# === Keep horizontal grid lines but remove y ticks and labels ===
plt.xlabel("")           # remove x-label
plt.ylabel("")           # remove y-label
yticks = np.arange(0, 65, 20)          # define tick positions
plt.yticks(yticks)                     # apply them so grid lines align at 0,20,40,60
plt.gca().set_yticklabels([])          # hide only labels
plt.tick_params(axis='y', length=0)    # hide tick marks
