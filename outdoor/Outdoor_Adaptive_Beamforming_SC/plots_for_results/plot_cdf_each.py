...
for speed_label, experiments in experiments_by_speed.items():
    ...
    for i, base_filename in enumerate(experiments):
        ...
        # delta_snr =   threshold - snr_values 
        # sorted_snr = np.sort(delta_snr)
        cdf = np.arange(1, len(sorted_snr) + 1) / len(sorted_snr)
        plt.plot(
            sorted_snr,
            cdf,
    ...
    plt.title(f"CDF of Threshold - SNR for {speed_label} Trials", fontsize=18, fontweight='bold')
    plt.xticks(np.arange(-25, 40, 5), fontsize=16, fontweight='bold')
    plt.yticks(fontsize=16, fontweight='bold')
    plt.grid(True)
    plt.legend(fontsize=16, loc='lower right')
    plt.tight_layout()

    filename = f"th_snr_delta_snr_cdf_{speed_label}"
