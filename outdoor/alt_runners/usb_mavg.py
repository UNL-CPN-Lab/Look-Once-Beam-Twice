...
def search_nearby_beams(center_beam, child_tx, child_rx, rx_streamer):
    ...
    rx_angle = rx_angle_from_index(center_beam)
    try:
    ...
    while True:
        ...
        for direction in [-1, 1]:
            ...
            rx_angle1 = rx_angle_from_index(new_beam)
            try:
                tx_index1 = tx_angle_index(-rx_angle1)
            except ValueError:
                print(f"[ERROR] RX angle {rx_angle1} not found in TX_BEAM_ANGLES.")
                tx_index1 = 32  # fallback
            run_interactive_command(child_tx, f'eder.tx.set_beam({tx_index1})')
        ...
        while True:
            ...
            if beam_index_str.isdigit():
                ...
                # if snr_yolo >= snr_percent_db(SNR_THRESHOLD_FACTOR, REFERENCE_MAX_SNR_DB):
                    ...
                    run_interactive_command(child_tx, f'eder.tx.set_beam({tx_index4})')
...
def main():
    ...
    #     print("[SUCCESS] Evaluation script completed.")rx
    # except subprocess.CalledProcessError as e:
