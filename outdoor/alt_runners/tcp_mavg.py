...
def search_nearby_beams(center_beam,  child_rx, rx_streamer, Txsock=None):
    ...
    if Txsock:
        ...
        try:
            ...
            print(f"[SOCKET] Sent SET_TX_BEAM:{tx_index2} to TX server.")
    ...
    while True:
        ...
        for direction in [-1, 1]:
            ...
            if Txsock:
                try:
                    Txsock.sendall(f"SET_TX_BEAM:{tx_index3}\n".encode())
                    print(f"[SOCKET] Sent SET_TX_BEAM:{tx_index3} to TX server.")
                except socket.timeout:
        ...
        while True:
            ...
            if beam_index_str.isdigit():
                ...
                # if snr_yolo >= snr_percent_db(SNR_THRESHOLD_FACTOR, REFERENCE_MAX_SNR_DB):
                    ...
                    if Txsock:
                        try:
                            Txsock.sendall(f"SET_TX_BEAM:{tx_index4}\n".encode())
                        except socket.timeout:
