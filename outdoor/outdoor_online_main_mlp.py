...
zmq_context = zmq.Context() # Initialize the ZMQ REQ socket for communication with the Jetson
...
model.load_state_dict(torch.load("mlp_models/offset_mlp_model_nh_outdoor.pt", map_location=device))
model.eval()


scaler = joblib.load("mlp_models/offset_scaler_nh_outdoor.pkl")
        ...
        while True:
            ...
            if Txsock:
                try:
                    ...
                    Txsock.recv_string()  # Wait for ACK
...
def main():
    ...
    # === Step 1: Create ZMQ REQ socket to remote TX server ===
