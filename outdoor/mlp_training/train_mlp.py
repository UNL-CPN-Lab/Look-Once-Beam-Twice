...
# This file lives in outdoor/mlp_training/. The MLP datasets + scalers + .pt
# weights live in outdoor/mlp_models/ (sibling folder).
...
def train_offset_model(dataset_path, num_epochs=200, batch_size=32, learning_rate=0.001, test_split=0.2):
    ...
    # Save artifacts
    torch.save(model.state_dict(), os.path.join(_MLP_MODELS_DIR, "offset_mlp_model_nh_outdoor.pt"))
    joblib.dump(scaler, os.path.join(_MLP_MODELS_DIR, "offset_scaler_nh_outdoor.pkl"))

    return model, scaler, y_test, y_pred, y

# Train
model, scaler, y_test, y_pred, y_all = train_offset_model(os.path.join(_MLP_MODELS_DIR, "offset_dataset_nh_outdoor.csv"))
