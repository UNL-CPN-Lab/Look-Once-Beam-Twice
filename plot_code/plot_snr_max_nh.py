...
# forward_df = pd.read_csv("/media/cse-vuran-32/mmWaveSSD/Nebraska_Hall/nh_forward_sweep_14db_3m/snr_tx32_rx32_forward.csv")

# backward_df = pd.read_csv("C:/Users/apramanik2/Desktop/research stuff/kw_exp_analysis_plots/backward_max_snr_per_angle.csv")

# Sort by angle for consistent plotting
forward_df = forward_df.sort_values(by="Rotor Angle")
# backward_df = backward_df.sort_values(by="Angle")
...
# Plot both lines
sns.lineplot(data=forward_df, x="Rotor Angle", y="Max SNR (dB)",
             marker="o", linewidth=3, label="Optimal(Full Exhaustive Forward Sweep)", color="darkblue")

# sns.lineplot(data=backward_df, x="Angle", y="Max SNR (dB)",
            #  marker="s", linewidth=3, label="Backward", color="firebrick")
...
plt.tight_layout()
plt.savefig("Forward_max_snr_vs_boresight.png", dpi=500)
plt.show()
