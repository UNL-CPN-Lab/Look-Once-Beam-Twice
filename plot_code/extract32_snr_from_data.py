...
output_csv = os.path.join(base_dir, "snr_tx32_rx32_forward.csv")
...
for folder in sorted(os.listdir(base_dir)):
    ...
    if os.path.isdir(folder_path) and os.path.exists(snr_file_path):
        try:
            ...
            df_filtered = df[(df["Tx Beam Angle"] == 0) & (df["Rx Beam Angle"] == 0)]
...
if snr_data_tx32_rx32:
    ...
    print(f"[SUCCESS] SNR summary for Tx32-Rx32 saved to: {output_csv}")
else:
