"""Orchestrator for the 5G NR baseline online indoor evaluations (continuous rotor).
...
`configurations/config.py` in place, and launches `continuous_online_main_5gnr.py`
for the live experiment. This folder benchmarks the 5G NR hierarchical
beamforming reference against the VIBE variants under matched conditions.
Console output is mirrored to `automatic_evaluation_terminal_log.txt`.

Run from this folder: `python3 automatic_5gnr_main.py`. See the folder README
...
ONLINE_SCRIPT = "continuous_online_main_5gnr.py"
...
def main():
    ...
    for snr_quantile, rotor_speed in itertools.product(SNR_QUANTILE, ROTOR_SPEEDS):
        ...
        gain = "8db"
        distance = "3m"
        test_number = f"online_5G{experiment_counter}"
