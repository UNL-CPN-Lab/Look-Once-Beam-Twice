"""VIBE-MA online runner — indoor, continuous rotor.

Launched once per experiment by `automatic_mavg_main.py`. Drives the rotor over
its sweep range while a YOLO service (running on `JETSON_IP:PORT`) returns
beam-index predictions for each rotor angle. For every prediction the runner
measures SNR on the predicted beam, then on `prediction + mean(history)`, then
falls back to a nearby-beam search; offsets that meet the SNR threshold feed
the moving-average history (`maxlen=10`). Per-step results are appended to
`Adaptive_Beamforming_SC/<experiment_name>/results_<experiment_name>.csv`.

Invoked as: `python3 continuous_online_main_mavg.py --test_number <id>`.
...
def search_nearby_beams(center_beam, child_tx, child_rx, rx_streamer):
            ...
            if snr_yolo >= SNR_THRESHOLD:
                ...
                if snr_adjusted >= SNR_THRESHOLD:
                    ...
                    selected_beam, final_snr_db, offset_error, max_power_dBm, beams_checked = search_nearby_beams(
                        adjusted_beam_index, child_tx, child_rx,  rx_streamer
                    )
