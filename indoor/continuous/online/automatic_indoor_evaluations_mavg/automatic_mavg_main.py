"""Orchestrator for VIBE-MA online indoor evaluations (continuous rotor).
...
`configurations/config.py` in place, and launches `continuous_online_main_mavg.py`
for the live experiment. Console output is mirrored to
`automatic_evaluation_terminal_log.txt`.

Run from this folder: `python3 automatic_mavg_main.py`. See the folder README
...
ONLINE_SCRIPT = "continuous_online_main_mavg.py"
...
def main():
    ...
    for snr_quantile, rotor_speed in itertools.product(SNR_QUANTILE, ROTOR_SPEEDS):
        ...
        gain = "8db"
        distance = "3m"
        test_number = f"QT_OAKD_MAVG{experiment_counter}"
        ...
        os.environ["GROUND_TRUTH_PATH"] = os.path.join(
            PROJECT_ROOT,
            "indoor", "continuous", "online", "automatic_indoor_evaluations_mavg",
