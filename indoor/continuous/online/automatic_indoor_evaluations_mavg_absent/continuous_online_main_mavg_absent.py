"""VIBE-MA ablation (no history fallback) online runner — indoor, continuous rotor.

Launched once per experiment by `automatic_mavg_main_absent.py`. Identical to
`continuous_online_main_mavg.py` except the moving-average history fallback is
removed: when the YOLO-predicted beam fails the SNR threshold the runner skips
the `prediction + mean(history)` step and goes straight to the nearby-beam
search. Used to isolate the contribution of the offset-history mechanism.
Per-step results are appended to
`Adaptive_Beamforming_SC/<experiment_name>/results_<experiment_name>.csv`.

Invoked as: `python3 continuous_online_main_mavg_absent.py --test_number <id>`.
        ...
        if beam_index_str.isdigit():
            ...
            # if snr_yolo >= snr_percent_db(SNR_THRESHOLD_FACTOR, REFERENCE_MAX_SNR_DB):
                ...
                selected_beam, final_snr_db, offset_error, max_power_dBm, beams_checked = search_nearby_beams(
                    Rx_center_beam_index, child_tx, child_rx,  rx_streamer
                )
