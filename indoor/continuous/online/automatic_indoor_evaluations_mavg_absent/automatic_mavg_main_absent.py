"""Orchestrator for VIBE-MA (history-fallback ablation) online indoor evaluations.
...
`configurations/config.py` in place, and launches
`continuous_online_main_mavg_absent.py` for the live experiment. This variant
is identical to VIBE-MA except the moving-average history fallback is disabled
(when SNR fails the threshold the runner skips the offset-corrected step and
goes straight to the neighbor search). Console output is mirrored to
`automatic_evaluation_terminal_log.txt`.

Run from this folder: `python3 automatic_mavg_main_absent.py`. See the folder
README for prerequisites (config, YOLO service, hardware).
...
ONLINE_SCRIPT = "continuous_online_main_mavg_absent.py"
...
def main():
    ...
    for snr_quantile, rotor_speed in itertools.product(SNR_QUANTILE, ROTOR_SPEEDS):
        ...
        gain = "8db"
        distance = "3m"
        test_number = f"QT_OAKD_MAVG_ABS{experiment_counter}"
        ...
        os.environ["GROUND_TRUTH_PATH"] = os.path.join(
            PROJECT_ROOT,
            "indoor", "continuous", "online", "automatic_indoor_evaluations_mavg_absent",
