"""VIBE-YOLOR online runner — indoor, continuous rotor.

Launched once per experiment by `automatic_basic_main.py`. Drives the rotor over
its sweep range while a YOLO service (running on `JETSON_IP:PORT`) returns
beam-index predictions for each rotor angle. The runner sets the RX beam
directly to the YOLO-predicted index, measures SNR, and logs the result. No
closed-loop refinement, no offset history, no neighbor search — VIBE-YOLOR
isolates the camera-priming + radio-projection stages of the VIBE pipeline.
Per-step results are appended to
`Adaptive_Beamforming_SC/<experiment_name>/results_<experiment_name>.csv`.

Invoked as: `python3 continuous_online_main_basic.py --test_number <id>`.
