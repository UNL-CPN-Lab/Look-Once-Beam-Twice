"""Orchestrator for VIBE-MLP online indoor evaluations (continuous rotor).
...
`configurations/config.py` in place, and launches `continuous_online_main_mlp.py`
for the live experiment. VIBE-MLP uses a learned 3-layer MLP to predict the
corrective beam offset (replacing the moving-average history of VIBE-MA).
Console output is mirrored to `automatic_evaluation_terminal_log.txt`.

Run from this folder: `python3 automatic_mlp_main.py`. See the folder README
for prerequisites (config, YOLO service, hardware, MLP weights).
...
ONLINE_SCRIPT = "continuous_online_main_mlp.py"
...
def main():
    ...
    for snr_quantile, rotor_speed in itertools.product(SNR_QUANTILE, ROTOR_SPEEDS):
        ...
        gain = "8db"
        distance = "3m"
        test_number = f"QT_OAKD_MLP{experiment_counter}"
        ...
        os.environ["GROUND_TRUTH_PATH"] = os.path.join(
            PROJECT_ROOT,
            "indoor", "continuous", "online", "automatic_indoor_evaluations_mlp",
