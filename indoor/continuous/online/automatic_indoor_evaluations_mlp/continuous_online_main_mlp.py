"""VIBE-MLP online runner — indoor, continuous rotor.

Launched once per experiment by `automatic_mlp_main.py`. Drives the rotor over
its sweep range while a YOLO service (running on `JETSON_IP:PORT`) returns
beam-index predictions for each rotor angle. Replaces VIBE-MA's moving-average
history with a learned 3-layer MLP (`OffsetMLP`): when the YOLO-predicted beam
fails the SNR threshold, the network takes
`[boresight, threshold_snr, yolo_beam, snr_yolo]` and predicts an offset; the
adjusted beam (`yolo_beam + offset`) is then evaluated. Per-step results are
appended to
`Adaptive_Beamforming_SC/<experiment_name>/results_<experiment_name>.csv`.

Requires trained MLP weights `offset_mlp_model_sc_jul23_oakd.pt` and matching
scaler `offset_scaler_sc_jul23_oakd.pkl` in this folder. The `.pt` file is not
bundled with the repo — train one with `train_mlp.py`.

Invoked as: `python3 continuous_online_main_mlp.py --test_number <id>`.
...
model.load_state_dict(torch.load("offset_mlp_model_sc_jul23_oakd.pt", map_location=device))
model.eval()


scaler = joblib.load("offset_scaler_sc_jul23_oakd.pkl")
...
def YOLO_RX(usrp, rx_streamer, child_tx, child_rx, experiment_dir):
    ...
    while currentrotorangle > 165 or currentrotorangle < 25:
        time.sleep(0.1)
    print("[YOLO_RX] Rotor in range. Starting main loop.")
    while currentrotorangle >= 25 and currentrotorangle <= 165:
        ...
        # Beamforming
        set_tx_beam_start_time = time.time()
        run_interactive_command(child_tx, f'eder.tx.set_beam({fixed_tx_beam})')
        set_tx_beam_time = time.time() - set_tx_beam_start_time
        
        set_rx_beam_start_time = time.time()
        run_interactive_command(child_rx, f'eder.rx.set_beam({Rx_center_beam_index})')
        set_rx_beam_time = time.time() - set_rx_beam_start_time
        
        stabilize_beam_start_time = time.time()
        recv_signal = np.zeros(SAMPLE_SIZE, dtype=np.complex64)
        for _ in range(30):
            rx_streamer.recv(recv_signal, metadata)
        stabilize_beam_time = time.time() - stabilize_beam_start_time

        measure_snr_start_time = time.time()
        ...
        measure_snr_time = time.time() - measure_snr_start_time
        if beam_index_str.isdigit():
        ...
        csv_data = {
            ...
            "Received Jetson inference Time": [received_jetson_inference_time],
            "Set TX Beam Time (s)": [set_tx_beam_time],
            "Set RX Beam Time (s)": [set_rx_beam_time],
            "Stabilize Beam Time (s)": [stabilize_beam_time],
            "Measure SNR Time (s)": [measure_snr_time]
