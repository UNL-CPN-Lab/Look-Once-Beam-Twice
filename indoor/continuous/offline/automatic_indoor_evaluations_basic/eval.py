...
# experiment_name = "sc_jul22_gain8db_3m_Off_redo_AB15" 

# === Paths and Constants ===
project_dir = os.path.join(PROJECT_ROOT, "indoor", "continuous", "offline", "automatic_indoor_evaluations_basic","Adaptive_Beamforming_SC")
...
summary_csv_path = os.path.join(summary_dir, f"quantile_based_offline_evaluation_experiments_results_summary_{date}.csv")
...
#     df_cleaned = df[(df["Jetson Detection"] != "NO_RADIO") & (df["SNR (dB)"].notna())]
...
try:
    ...
    # SNR_THRESHOLD = float(metadata.get("Threshold", "nan"))
    # From full sweep SNR values:
    # 70th percentile: 22.29 dB
    # 80th percentile: 23.22 dB
    # 90th percentile: 24.28 dB
    # SNR_THRESHOLD = 22.29
