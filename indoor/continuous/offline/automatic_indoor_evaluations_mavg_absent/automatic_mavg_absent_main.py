"""Orchestrator for VIBE-MA (history-fallback ablation) offline indoor evaluations.
...
`continuous_offline_main_mavg_absent.py` for the live experiment. This variant
...
Run from this folder: `python3 offline_automatic_mavg_absent_main.py`. See the
folder README for prerequisites (pre-collected ground truth, hardware, config).
...
ONLINE_SCRIPT = "continuous_offline_main_mavg_absent.py"
...
def main():
        ...
        for snr_quantile, rotor_speed in itertools.product(SNR_QUANTILE, ROTOR_SPEEDS):
            ...
            os.environ["GROUND_TRUTH_PATH"] = os.path.join(
                PROJECT_ROOT,
                "indoor", "continuous", "offline", "automatic_indoor_evaluations_mavg_absent",
            ...
            test_number = f"off_OAKD_MAVG_ABS{experiment_counter}"
