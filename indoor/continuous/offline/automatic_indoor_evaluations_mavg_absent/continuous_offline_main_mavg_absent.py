"""VIBE-MA ablation (no history fallback) offline runner — indoor, continuous rotor.

Launched once per experiment by `offline_automatic_mavg_absent_main.py`. Same
shape as the online VIBE-MA-absent runner (YOLO → neighbor search, no history
fallback) except SNR is looked up in a pre-collected ground-truth CSV
(`GROUND_TRUTH_CSV` from config.py) instead of being measured live. The
hardware (Sivers TX/RX, USRP, rotor) is still driven so the experiment
behaves identically from the runtime side; only the SNR observation is
replayed. Used to isolate the contribution of the offset-history mechanism.
Per-step results are appended to
`Adaptive_Beamforming_SC/<experiment_name>/results_<experiment_name>.csv`.

Invoked as: `python3 continuous_offline_main_mavg_absent.py --test_number <id>`.
    ...
    while currentrotorangle >= 25 and currentrotorangle <= 160:
        ...
        if beam_index_str.isdigit():
            ...
            if snr_yolo is not None and snr_yolo >= SNR_THRESHOLD:
                ...
                selected_beam, final_snr_db, offset_error, beams_checked = search_nearby_beams(Rx_center_beam_index)
