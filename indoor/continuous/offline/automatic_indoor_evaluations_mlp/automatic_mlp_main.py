"""Orchestrator for VIBE-MLP offline indoor evaluations (continuous rotor).
...
`continuous_offline_main_mlp.py` for the live experiment. VIBE-MLP uses a
learned 3-layer MLP to predict the corrective beam offset (replacing the
moving-average history of VIBE-MA). Console output is mirrored to
`offline_automatic_evaluation_terminal_log.txt`.

Run from this folder: `python3 offline_automatic_mlp_main.py`. See the folder
README for prerequisites (pre-collected ground truth, hardware, MLP weights).
...
logfile_name = "offline_automatic_evaluation_terminal_log.txt"
...
# 70th percentile SNR_th: 30.61 dB
# 80th percentile SNR_th: 33.42 dB
# 90th percentile SNR_th: 36.79 dB
# 95th percentile SNR_th: 38.22 dB
# 99th percentile SNR_th: 39.74 dB

# === SCRIPT PATHS ===
GROUND_SWEEP_SCRIPT = "optimized_beam_sweep.py"
ONLINE_SCRIPT = "continuous_offline_main_mlp.py"
...
def main():
        ...
        for snr_quantile, rotor_speed in itertools.product(SNR_QUANTILE, ROTOR_SPEEDS):
            ...
            test_number = f"off_OAKD_MLP{experiment_counter}"
