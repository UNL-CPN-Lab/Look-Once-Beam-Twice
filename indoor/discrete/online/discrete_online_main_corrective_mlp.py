...
model.load_state_dict(torch.load("offset_mlp_model_sc.pt", map_location=device))
model.eval()


scaler = joblib.load("offset_scaler_sc.pkl")
...
# Function to get the Rx beam index from the angle
    ...
    while True:    
        ...
        if beam_index_str.isdigit():
            ...
            if previous_rotor_angle is not None and currentrotorangle != previous_rotor_angle:
             ...
             # Assign default safe values to avoid UnboundLocalError
                selected_beam = Rx_center_beam_index
                final_snr_db = None
                offset_error = None
                beams_checked = 1
                adjustment_type = "NoRadio"
        ...
        if beam_index_str.isdigit():
            ...
            if snr_yolo >= snr_percent_db(SNR_THRESHOLD_FACTOR, REFERENCE_MAX_SNR_DB):
                ...
                print(f"[YOLO_THREAD] Beamformed at YOLO beam index {Rx_center_beam_index} with SNR {snr_yolo:.2f} dB.")
            else:
                ...
                boresight_gt = int(currentrotorangle) 
                ...
                adjusted_beam_index = yolo_predicted_beam + predicted_offset
...
def main():
    ...
    metadata_content = {
        ...
        "Algorithm": "Adaptive Beamforming + Corrective MLP",
