"""TX × RX SNR heatmap rendering for the full beam sweep.

Companion to `Sivers_BeamSweep_Delay_Optimized_SNR.py` (and the related
`calibration_with_sa.py`): given the directory written by a sweep run, reads
`snr_data.csv` (`Sample size, Tx Beam, Rx Beam, SNR (dB)` per row), reshapes
into a 64 × 64 matrix, and saves a heatmap PNG annotated with the best beam
pair.

Public function:
- `plotheatmap(sweep_directory_path, samples_per_beam)` — reads the per-pair
  CSV from `<sweep_directory_path>/snr_data.csv` and writes a heatmap PNG
  named `<basename(sweep_directory_path)>_heatmap.png` in the same directory.

The sweep script imports this module as `heatmap` and calls
`heatmap.plotheatmap(...)` after data collection finishes.
...
from configurations.utils import calculate_power_metrics
import os
...
def plotheatmap(sweep_directory_path,samples_per_beam):
    ...
    ax = sns.heatmap(max_signal_powers, cmap=custom_black_blue_red, vmin=-80, vmax=-10, cbar_kws={'label': 'Rx Power(dBm)', 'pad': 0.02})
...
# Example Usage:
sweep_directory_path = "<DATA_ROOT>/mmWaveSSD/Schorr_Center/Full_Sweep_Experiment/sweepData/sc_jul17_gain10db_3m_t1" # linux
