"""Full TX × RX beam sweep with spectrum-analyzer (SA) reference power.

Same per-pair sweep structure as `Sivers_BeamSweep_Delay_Optimized_SNR.py`,
but the IQ-derived SNR is paired with a spectrum-analyzer measurement for
absolute-power calibration. Used to convert the USRP's raw dBm scale into a
calibrated received-power scale that can be compared across experiments.

Two modes (selected interactively at startup):
- `'F'` — Full Sweep: 64 TX × 64 RX.
- `'S'` — Single Sweep: one chosen `tx_beam`, 64 RX.

Outputs under `main_directory` mirror the standard sweep tooling (per-pair
CSV, iteration timestamps, raw IQ, per-TX `.dat` buffers, heatmap PNG) plus
the SA-derived absolute-power column.

Run: `python3 calibration_with_sa.py` (from this folder). Requires the SA
instrument to be powered and reachable (see your lab's SA driver setup).
...
def rx_host(usrp, rx_streamer, start_time, child_rx, child_tx, sweep_mode, tx_beam, experiment_dir, SAMPLE_SIZE):
    ...
    m = 5
    while (m>0):
            tx_command = 'eder.tx.set_beam({})'.format(32)
            run_interactive_command(child_tx, tx_command)
            print("")
            
            rx_command = 'eder.rx.set_beam({})'.format(32)
...
def main(tx_streamer, rx_streamer, sweep_mode, experiment_name, tx_beam, experiment_dir):
    ...
    run_interactive_command(child_rx, 'eder.regs.wr(\'rx_gain_ctrl_bb2\',0xFF)')
